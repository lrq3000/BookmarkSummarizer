...
class TestCrawlCoverage(unittest.TestCase):
    def setUp(self):
        ...
        crawl.current_lmdb_map_size = 10485760
    ...
    def test_sanitize_bookmark_complex(self):
        ...
        b1 = {"driver": MockDriver(), "ok": 1}
        s1 = crawl.sanitize_bookmark(b1)
        self.assertNotIn("driver", s1)
        self.assertEqual(s1["ok"], 1)

        class Complex:
            pass
        c = Complex()
        b2 = {"complex": c, "ok": 1}
        s2 = crawl.sanitize_bookmark(b2)
        self.assertNotIn("complex", s2)

        b3 = {"list": [{"a": 1}, {"b": MockDriver()}]}
        s3 = crawl.sanitize_bookmark(b3)
        self.assertEqual(s3["list"][0]["a"], 1)
        self.assertNotIn("b", s3["list"][1])

        b4 = {}
        b4["self"] = b4
        s4 = crawl.sanitize_bookmark(b4)
        self.assertEqual(s4, {'self': None})

    def test_safe_pickle(self):
        obj = {"a": 1}
        p = crawl.safe_pickle(obj)
        self.assertEqual(pickle.loads(p), obj)

    # --- Disk Space Tests ---
    @patch('shutil.disk_usage')
    def test_check_disk_space_success(self, mock_disk_usage):
        mock_disk_usage.return_value = MagicMock(free=1024**3)
        self.assertTrue(crawl.check_disk_space())
    ...
    @patch('lmdb.open')
    def test_resize_lmdb_database_logic(self, mock_open):
        mock_env = MagicMock()
        mock_open.return_value = mock_env
        crawl.lmdb_storage_path = "test.lmdb"
        ...
        success, size = crawl.resize_lmdb_database(100, max_attempts=1)
        self.assertFalse(success)
        self.assertEqual(size, 100)
        self.assertEqual(mock_open.call_count, 2)

    @patch('crawl.resize_lmdb_database')
    def test_safe_lmdb_operation_resize_flow(self, mock_resize):
        # 1. Resize Success, Retry Success
        crawl.lmdb_env = MagicMock()
        ...
        # 2. Resize Success, Retry Failure
        op.side_effect = [lmdb.MapFullError("Full"), Exception("RetryFail")]
        crawl.safe_lmdb_operation(op, readonly=False)
        self.assertTrue(crawl.use_fallback)

        # 3. Resize Failure
        crawl.use_fallback = False
        op.side_effect = lmdb.MapFullError("Full")
        mock_resize.return_value = (False, 100)
        crawl.safe_lmdb_operation(op, readonly=False)
        self.assertTrue(crawl.use_fallback)

    def test_safe_lmdb_operation_all_exceptions(self):
        errors = [lmdb.MapResizedError("e"), lmdb.DiskError("e"), lmdb.InvalidError("e"),
                  lmdb.BadTxnError("e"), lmdb.BadRslotError("e"), lmdb.BadValsizeError("e"),
                  Exception("Generic")]
        ...
        for err in errors:
            ...
            crawl.safe_lmdb_operation(op)
            self.assertTrue(crawl.use_fallback, f"Failed for {type(err)}")
            crawl.use_fallback = False
    ...
    @patch('crawl.create_session')
    def test_fetch_webpage_content_print_error(self, mock_session, mock_print):
        bookmark = {"url": "http://u", "name": "Title"}
        mock_print.side_effect = [UnicodeEncodeError('ascii', 'Title', 0, 1, 'bad'), None, None, None, None]
    ...
    @patch('crawl.fetch_webpage_content')
    def test_parallel_fetch_deduplication(self, mock_fetch):
        bookmarks = [{"url": "u1", "name": "n1", "type": "url"}]
        crawl.lmdb_env = MagicMock()
        mock_txn = MagicMock()
        crawl.lmdb_env.begin.return_value = mock_txn
        mock_txn.__enter__.return_value = mock_txn
    ...
    @patch('crawl.fetch_webpage_content')
    def test_parallel_fetch_worker_exception(self, mock_fetch):
    ...
    def test_main_branches(self, mock_prep, mock_parallel, mock_get_bookmarks, mock_init, mock_load_config, mock_args):
        ...
        # Test failed API connection in main
        with patch('crawl.test_api_connection', return_value=False):
             mock_args.return_value.no_summary = False # Try to generate summary
             mock_args.return_value.from_json = False
             mock_get_bookmarks.return_value = [{"url": "u1", "name": "n", "type": "url"}]
             mock_parallel.return_value = ([{"url": "u1", "content": "c"}], [], 1)

             crawl.main()
             # Should print "Skipping summary..."

    @patch('crawl.get_bookmarks', return_value=[])
    @patch('crawl.init_lmdb')
    @patch('crawl.parallel_fetch_bookmarks')
    @patch('crawl.prepare_webdriver')
    def test_main_stats_and_fail_save(self, mock_prep, mock_parallel, mock_init, mock_get):
        bookmarks_result = [
            {"url": "u1", "content": "c1", "content_length": 10, "crawl_method": "requests"},
            {"url": "u2", "content": "c2", "content_length": 20, "crawl_method": "selenium"}
        ]
        failed_result = [{"url": "u3", "reason": "fail"}]
        mock_parallel.return_value = (bookmarks_result, failed_result, 2)

        with patch('crawl.safe_lmdb_operation') as mock_safe:
            ...
            with patch('crawl.parse_args') as mock_args:
                mock_args.return_value = MagicMock(
                    limit=0, workers=1, no_summary=True, browser=None, profile_path=None, config="c",
                    rebuild=False, flush_interval=60, lmdb_map_size=None, lmdb_max_dbs=None, lmdb_readonly=False,
                    lmdb_resize_threshold=0.8, lmdb_growth_factor=2.0, enable_backup=False, disable_backup=True,
                    backup_dir=".", backup_on_failure_stop=False, min_delay=0, max_delay=0, parsers=None,
                    skip_unreachable=False, from_json=False
                )
    ...
    @patch('crawl.parallel_fetch_bookmarks')
    @patch('crawl.prepare_webdriver')
    def test_main_rebuild(self, mock_prep, mock_parallel, mock_init, mock_get):
        with patch('crawl.parse_args') as mock_args:
            mock_args.return_value = MagicMock(rebuild=True, config="c", enable_backup=False)
            mock_args.return_value.limit = 0
            mock_args.return_value.workers = 1
            mock_args.return_value.no_summary = True
            mock_args.return_value.disable_backup = True
            mock_args.return_value.parsers = None
            mock_args.return_value.from_json = False
            mock_parallel.return_value = ([], [], 0)
    ...
    def test_signal_handler(self):
        with patch('crawl.cleanup_lmdb') as mock_clean:
