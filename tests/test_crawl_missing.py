...
class TestCrawlMissing(unittest.TestCase):
    ...
    def test_safe_lmdb_operation_fallback(self):
        ...
        result = crawl.safe_lmdb_operation(None, fallback_func)
        self.assertEqual(result, "fallback")

        # Test fallback failure
        fallback_func.side_effect = Exception("Fallback failed")
        result = crawl.safe_lmdb_operation(None, fallback_func)
    ...
    def test_safe_lmdb_operation_resize_flow(self):
        # Setup
        ...
        # Operation that fails first then succeeds
        operation_func = MagicMock(side_effect=[crawl.lmdb.MapFullError("Full"), "Success"])

        with patch('crawl.resize_lmdb_database', return_value=(True, 200)) as mock_resize:
            result = crawl.safe_lmdb_operation(operation_func, readonly=False)

            self.assertEqual(result, "Success")
            mock_resize.assert_called_with(100, 2.0)
            self.assertEqual(operation_func.call_count, 2)
    ...
    def test_safe_lmdb_operation_all_exceptions(self):
        exceptions = [
            ...
            crawl.lmdb.DiskError("Disk"),
            crawl.lmdb.InvalidError("Invalid"),
            crawl.lmdb.BadTxnError("BadTxn"),
            crawl.lmdb.BadRslotError("BadRslot"),
            crawl.lmdb.BadValsizeError("BadValsize"),
            Exception("Generic")
        ]
    ...
    def test_safe_lmdb_operation_resize_failures(self):
        op_func = MagicMock(side_effect=crawl.lmdb.MapFullError("Full"))
        crawl.lmdb_env = MagicMock()

        # Case 1: Readonly
        crawl.use_fallback = False
        crawl.safe_lmdb_operation(op_func, readonly=True)
        self.assertTrue(crawl.use_fallback)

        # Case 2: No current size
        crawl.use_fallback = False
        crawl.current_lmdb_map_size = None
        crawl.safe_lmdb_operation(op_func, readonly=False)
        self.assertTrue(crawl.use_fallback)
        ...
        # Case 4: Resize succeeds but retry fails
        crawl.use_fallback = False
        with patch('crawl.resize_lmdb_database', return_value=(True, 200)):
             op_func.side_effect = [crawl.lmdb.MapFullError("Full"), Exception("Retry fail")]
             crawl.safe_lmdb_operation(op_func, readonly=False)
             self.assertTrue(crawl.use_fallback)
